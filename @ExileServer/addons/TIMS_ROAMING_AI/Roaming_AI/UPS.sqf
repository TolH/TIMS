...
if (isNil("KRON_UPS_INIT")) then {
	...
	KRON_OnRoad = {private["_pos","_car","_tries","_lst"];_pos=_this select 0; _car=_this select 1; _tries=_this select 2; _lst=_pos nearRoads 5; if ((count _lst!=0) && (_car || !(surfaceIsWater _pos))) then {_tries=99}; (_tries+1)};
	...
	KRON_deleteDead = {private["_u","_s"];_u=_this select 0; _s= _this select 1; _u removeAllEventHandlers "killed"; uiSleep _s; deletevehicle _u};
...
while {_loop} do {
	...
	if ((_issoldier) && ((count _enemies)>0) && !(_exit)) then {
		...
		if (_opfknowval>_lastknown || _gothit) then {
			...
			//THROW SMOKE TEST
			/*_SmokeGrenade_Color_Array = ["SmokeShell", "SmokeShellRed", "SmokeShellGreen", "SmokeShellYellow", "SmokeShellBlue", "SmokeShellOrange"];
			_SmokeGrenade_Color = selectRandom _SmokeGrenade_Color_Array;
			_smokeGrenade = _SmokeGrenade_Color createVehicle getPos _npc;
			_npc fire ["SmokeShellMuzzle","SmokeShellMuzzle",_SmokeGrenade_Color];*/
		};
	...
	if !(_newpos) then {
		...
		if (_waiting<=0) then {
			...
			if (_makenewtarget) then {
				if ((_nomove=="NOMOVE") && (_timeontarget>ALERTTIME)) then {
					...
					while {((([_currPos,_targetPos] call KRON_distancePosSqr) < _mindist)) && (_tries<20)} do {
						...
						while {_tries<20} do {
							...
							if (_iscar) then {
								_roadlist = _targetPos nearRoads 150;
				...
				if ((_pause!="NOWAIT") && (_waiting<0)) then {_waiting = (5 + random 5)};
			};
		};
	};
	uiSleep .01;
	...
	{
		//UNIT NOT IN A VEHICLE
		if (_npc == (vehicle _npc) && !_swimming) then 
		{
			_drydist=999;
			// look around, to find a dry spot
			for [{_a=0}, {_a<=270}, {_a=_a+90}] do {
				_dp=[_currPos,400,_a] call KRON_relPos; 
		...
		/*if (_isman && !_swimming) then 
		{
			...
			// look around, to find a dry spot
			for [{_a=0}, {_a<=270}, {_a=_a+90}] do {
				_dp=[_currPos,100,_a] call KRON_relPos; 
				if !(surfaceIsWater _dp) then {_targetPos=_dp};
			};
			_newpos=true; 
			_swimming=true;
