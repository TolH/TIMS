/*
		...
		Calling the script:
			...
			default: 		nul = [this] execVM "TIMS\LV\heliParadrop.sqf";
			...
			custom: 		nul = [spot, side, allowDamage, captive, distance, direction, flyby, fly height, jump distance,
								...
								group, custom init, ID, MP, doors, classes] execVM "TIMS\LV\heliParadrop.sqf";
...
EXAMPLE: 	nul = [player, 2, false, true, 1000, "random", true, 500, 200, 6, 1, 50, true, false, true, true, player, false, 0.75, nil, nil, 1,false] execVM "TIMS\LV\heliParadrop.sqf";
*/
...
"_RandomLauncherChance","_newGroupOPFOR"];
...
_captive = param [3,true];
_heliDistance = param [4,1000];
_heliDirection = param [5,"random"];
_flyBy = param [6,true];
_flyHeight = param [7,100];
_jumpDistanceFromTarget = param [8,250];
_jumperAmount = param [9,5];
_jumpDelay = param [10,1];
_openHeight = param [11,60];
_smokes = param [12,true];
...
_classModuleFilters = param [24,["TOHL"]];
...
[_grp,2] setWaypointCompletionRadius 5000;
...
for "_i" from 0 to 0 do {
	...
	_man2 = _grp2 createUnit [_man1, getPos _heli, [], 0, "NONE"];
...
for "_i" from 1 to _jumperAmount step 1 do{
	...
	[_man2] joinSilent _newGroupOPFOR;
	//_man2 addMPEventHandler ["MPKilled", {_this call TIMS_AI_MPKilled;}];
	if(typeName _skills != "STRING")then{_skls = [_man2,_skills] call LV_ACskills;};
	if(!isNil("_customInit"))then{ 
		[_man2,_customInit] spawn LV_vehicleInit;
	};
	...
	if (_RandomLauncherChance <= AI_ROCKET_EQUIP_CHANCE) then 
	{
		...
		_man2 addBackpack "B_Carryall_oucamo";
		_man2 addWeapon "launch_RPG32_F";
		_man2 addMagazine "RPG32_F";
		_man2 addMagazine 'RPG32_HE_F';
				...
				if((_target select 0) == "PATROL")then{ ////USE patrol-vD.sqf
					{
						...
						nul = [_x,_cPosition,_cRadius,_doorHandling] execVM "TIMS\LV\patrol-vD.sqf";
