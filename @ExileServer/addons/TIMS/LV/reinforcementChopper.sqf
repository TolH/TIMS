/*
		...
		Calling the script:
			...
			default: 		nul = [this] execVM "TIMS\LV\reinforcementChopper.sqf";
			custom: 		nul = [spot,exact,side,type,captive,patrol,target,direction,distance,precise,
								cycle,groupSize,skills,smoke,group,custom init,ID,MP,doors,classes] execVM "TIMS\LV\reinforcementChopper.sqf";
	...
	EXAMPLE: 	nul = [player,false,2,3,false,true,player,"random",1000,true,false,8,0.75,[false,true,false,true],nil,nil,33,false,false,["ALL"]] execVM "TIMS\LV\reinforcementChopper.sqf";
*/
...
_classModuleFilters = param [19,["TOHL"]];
...
_range = _distance;
_pos = [(_targetPos select 0) + (sin _dir) * _range, (_targetPos select 1) + (cos _dir) * _range, 100];
