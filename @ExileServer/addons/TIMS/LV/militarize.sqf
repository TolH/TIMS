/*
		...
		default: 	nul = [this] execVM "TIMS\LV\militarize.sqf";
		
		custom:		nul = [target, side, radius, spawn men, spawn vehicles, still, men ratio, vehicle ratio, 
							skills, group, custom init, ID, smokes, doors, classes] execVM "TIMS\LV\militarize.sqf";
...
EXAMPLE: nul = [this,2,50,[true,true],[true,false,true],false,[10,0],0.1,[0.2,0.2,0.2,0.85,0.9,0.75,0.1,0.6,1,1],nil,nil,nil,true,true,["ALL"]] execVM "TIMS\LV\militarize.sqf";
*/
...
private ["_divers","_landVeh","_newGroupOPFOR","_waterVeh","_airVeh","_mRatio","_vRatio","_mGroup","_classModule","_classModuleFilters"];
_cPos = param [0];
_side = param [1,2];
_radius = param [2,150];
...
if((_men select 0)||(_men select 1))then{
	for "_i" from 1 to _menAmount do{
		...
		_unit = _newGroupOPFOR createUnit [_unitType, _pos, [], 0, "NONE"];
		//SIDE TEST CHANGE
		[_unit] joinSilent _newGroupOPFOR;
		_unit setPos _pos;
		if(!_still)then{
			if(_unitType in _menArray)then{
				nul = [_unit,_cPos,_radius,_doorHandling] execVM "TIMS\LV\patrol-vD.sqf";
			}else{
				nul = [_unit,_pos] execVM 'TIMS\LV\patrol-vH.sqf';
			};
		};
			...
			if(true)then{ //side > 0
				if(surfaceIsWater _pos)then{
					if(_vehicles select 1)then{
						...
						if(!_still)then{nul = [vehicle _driver,_pos] execVM 'TIMS\LV\patrol-vH.sqf';};
						_validPos = true;
					}else{
						_wMode = 0;
					};
					...
					if((_vehicles select 0)&&(_vehicles select 2))then{
						...
						if(_whichOne < 3)then{
							...
							if(!_still)then{nul = [_driver,_pos,[200,200]] execVM 'TIMS\LV\patrol-vE.sqf';};
							vehicle _driver flyInHeight 10;
							_validPos = true;
						}else{
							_driver = [_pos, (_side),_classModuleFilters,nil,nil,false] call LV_fullLandVehicle;
							if(!_still)then{nul = [vehicle _driver,_pos] execVM 'TIMS\LV\patrol-vE.sqf';};
							_validPos = true;
						};
					}else{
