	...
	if((count _pincode) isEqualTo (count _objectPinCode))then
	{
		if(_pincode isEqualTo _objectPinCode)then
		{
			if!(_state)then
			{
				...
				_locklog = format ["( %1 ) %2 UNLOCKED %3 @ %4 - PIN IS %5",_player,getPlayerUID _player,_object,mapGridPosition _object,_pinCode];
				["LOCKLOG",_locklog] call FNC_A3_CUSTOMLOG;
			}
			else
			{
				if(isNumber(configFile >> "CfgVehicles" >> _type >> "exileIsLockable"))then
				...
				_locklog = format ["( %1 ) %2 LOCKED %3 @ %4 - PIN IS %5",_player,getPlayerUID _player,_object,mapGridPosition _object,_pinCode];
				["LOCKLOG",_locklog] call FNC_A3_CUSTOMLOG;
			};
